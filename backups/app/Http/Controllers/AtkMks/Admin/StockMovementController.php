...
class StockMovementController extends Controller
{
    public function index(Request $request)
    {
        $movements = AtkStockMovement::with(['item', 'createdBy'])
            ->whereHas('item', fn ($query) => $query->forModule(AtkItem::MODULE_ATK_MKS))
            ...
            ->paginate(20)
            ->withQueryString();
        $requestSources = AtkRequest::forModule(AtkRequest::MODULE_ATK_MKS)->whereIn(
            'id',
        ...
        $items = AtkItem::forModule(AtkItem::MODULE_ATK_MKS)->orderBy('name')->get();

        return view('atk-mks.admin.stock_movements.index', compact('items', 'movements', 'requestSources'));
    }
}
