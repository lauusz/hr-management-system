<x-app title="Tambah User Approval">
    <x-slot name="header">
        <div class="approval-create-header">
            <a href="{{ route('hr.approvers.show', $approver) }}" class="approval-create-back">Kembali</a>
            <div class="section-header-inline">
                ...
                <div>
                    <h1 class="section-title">Tambah User Approval</h1>
                    <p class="section-subtitle">Tambahkan user ke mapping {{ $roleLabel }} {{ $approver->name }}.</p>
                </div>
            </div>
        </div>
    </x-slot>
    ...
    <section class="approval-add-profile">
        <div>
            <div class="approval-add-name">{{ $approver->name }}</div>
            <div class="approval-add-meta">
    ...
    <form method="POST" action="{{ route('hr.approvers.users.store', $approver) }}" class="approval-create-panel" data-approver-user-form>
        @csrf

        <div class="approval-create-field">
            <label for="approver-user-search">Cari nama user</label>
            <div class="approval-create-autocomplete">
                <input type="hidden" name="user_id" value="{{ old('user_id') }}" data-approver-user-selected-id>
                <input
                    id="approver-user-search"
                    type="search"
                ...
                >
                <ul id="approver-user-suggestions" class="approver-suggestions" role="listbox" aria-label="Saran user approval">
                    ...
                    @endforeach
                    <li class="approver-suggestion-empty" data-approver-user-no-result hidden>Tidak ada user yang cocok.</li>
                </ul>
            </div>
            @error('user_id')
                <p>{{ $message }}</p>
        ...
        @if($candidates->isEmpty())
            <div class="approval-create-empty">Tidak ada user aktif yang tersedia untuk ditambahkan ke mapping ini.</div>
        @endif

        <div class="approval-create-actions">
            <a href="{{ route('hr.approvers.show', $approver) }}">Batal</a>
    ...
    <script>
        $(function() {
            ...
            const $list = $('#approver-user-suggestions');
            const $options = $('[data-approver-user-option]');
            const $emptyState = $('[data-approver-user-no-result]');
            let activeIndex = -1;
