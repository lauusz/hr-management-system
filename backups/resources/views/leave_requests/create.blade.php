<x-app title="Buat Pengajuan Izin">
    ...
    @php
        ...
        foreach (\App\Enums\LeaveType::cases() as $case) {
            ...
            if ($case->value === \App\Enums\LeaveType::OFF_SPV->value && $offRemaining !== null) { $hint = 'Sisa ' . $offRemaining . ' hari · Sabtu saja'; }
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (typeof flatpickr === 'function' && rangeInput) {
                var leaveCalendar = flatpickr(rangeInput, {
                    mode: 'range',
                    dateFormat: 'Y-m-d',
                    allowInput: true,
                    ...
                    disable: [function(date) {
                        ...
                        return selectedType && selectedType.value === 'OFF_SPV' && date.getDay() !== 6;
                    }],
                    onChange: function(selectedDates, dateStr) {
                        ...
                        var selectedType = document.querySelector('input[name="type"]:checked');
                        if (selectedType && selectedType.value === 'OFF_SPV' && selectedDates.length === 1) {
                            leaveCalendar.close();
                        }
                    }
                });
                document.querySelectorAll('input[name="type"]').forEach(function(radio) {
                    radio.addEventListener('change', function() { leaveCalendar.redraw(); });
                });
            }
        });
    </script>
