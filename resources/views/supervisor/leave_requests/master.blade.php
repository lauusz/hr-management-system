<x-app title="Rekap Cuti Bawahan">
    ...
    <div class="card mb-4">
        <div class="card-header-simple">
            ...
            <p class="card-subtitle-sm">Cari riwayat pengajuan izin dan cuti staf di bawah supervisi Anda.</p>
        </div>
        
        {{-- Form Filter --}}
        <form method="GET" action="{{ route('supervisor.leave.master') }}" class="filter-container">
            {{-- Filter Tanggal --}}
            ...
            <div class="filter-group">
                ...
                <select name="type" class="form-control">
                    ...
                    @foreach($typeOptions as $case)
                        @php
                            $val = $case->value;
                            // Normalisasi label Cuti Khusus agar seragam
            ...
            @php
                $statusLabels = [
                    \App\Models\LeaveRequest::PENDING_SUPERVISOR => 'Menunggu Approval', // General label
            ...
            <div class="filter-actions">
                ...
                <a href="{{ route('supervisor.leave.master') }}" class="btn-reset">Reset</a>
                @endif
            </div>
        </form>
    </div>
            ...
            <table class="custom-table">
                ...
                <tbody>
                    @forelse($items as $i => $row)
                        @php
                            // Logic Badge Status (Konsisten dengan HR Master)
                            $st = $row->status;
                        ...
                        <tr>
                            ...
                            <td class="text-right">
                                {{-- Arahkan ke Detail Approval (karena view detailnya sama) --}}
                                <a href="{{ route('approval.show', $row) }}" class="btn-action">
                                    Detail
                                </a>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="7" class="empty-state">
                                Belum ada riwayat pengajuan dari bawahan Anda.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
