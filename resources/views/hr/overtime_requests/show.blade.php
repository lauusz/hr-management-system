<x-app title="Detail Pengajuan Lembur (HR)">
        ...
        <div class="card">
            <div class="profile-header">
                ...
                @php
                    ...
                    if ($overtimeRequest->status == \App\Models\OvertimeRequest::STATUS_PENDING_SUPERVISOR) {
                        ...
                        $statusLabel = '❌ Ditolak';
                    } elseif ($overtimeRequest->status == \App\Models\OvertimeRequest::STATUS_CANCELLED) {
                        $statusBadge = 'badge-red';
            ...
            <div class="action-footer">
                <div class="left-action">
                    <a href="{{ route('hr.overtime-requests.index') }}" class="btn-modern btn-back">
                ...
                <div class="right-action">
                    @if($overtimeRequest->status != \App\Models\OvertimeRequest::STATUS_CANCELLED)
                        {{-- [EDIT & CANCEL BUTTONS] --}}
                        ...
                        @if($overtimeRequest->status != \App\Models\OvertimeRequest::STATUS_APPROVED_HRD && $overtimeRequest->status != \App\Models\OvertimeRequest::STATUS_REJECTED && $overtimeRequest->status != \App\Models\OvertimeRequest::STATUS_PENDING_SUPERVISOR)
    ...
    <form action="{{ route('overtime-requests.update', $overtimeRequest->id) }}" method="POST">
        ...
        <div style="max-height: 70vh; overflow-y: auto; padding-right: 5px;">
            <div class="form-group">
                ...
                <input type="date" name="overtime_date" class="form-control" value="{{ $overtimeRequest->overtime_date->format('Y-m-d') }}" required>
            </div>

            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px; margin-bottom: 12px;">
                <div class="form-group" style="margin-bottom:0;">
                    <label class="lbl-edit">Jam Mulai</label>
                    <input type="time" name="start_time" class="form-control" value="{{ $overtimeRequest->start_time->format('H:i') }}" required>
                </div>
                <div class="form-group" style="margin-bottom:0;">
                    <label class="lbl-edit">Jam Selesai</label>
                    <input type="time" name="end_time" class="form-control" value="{{ $overtimeRequest->end_time->format('H:i') }}" required>
                </div>
            </div>

            <div class="form-group">
                <label class="lbl-edit">Keterangan / Deskripsi</label>
                <textarea name="description" rows="3" class="form-control" required style="resize: vertical; min-height: 80px;">{{ $overtimeRequest->description }}</textarea>
            </div>
        </div>
...
<x-modal id="modal-delete" title="Batalkan Pengajuan?" type="form">
    ...
    <style>
        ...
        .form-control { width: 100%; padding: 8px; border: 1px solid #d1d5db; border-radius: 6px; font-size: 13.5px; transition: border-color 0.2s; }
