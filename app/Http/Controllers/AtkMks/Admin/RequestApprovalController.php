...
class RequestApprovalController extends Controller
{
    public function index(Request $request)
    {
        ...
        return view('atk-mks.admin.requests.index', compact('requests'));
    }

    public function createManual()
    {
        $users = User::query()->active()->with('profile.pt')->orderBy('name')->get();
        $items = AtkItem::query()->forModule(AtkItem::MODULE_ATK_MKS)->available()->orderBy('name')->get();

        return view('atk-mks.admin.requests.manual-create', compact('users', 'items'));
    }
    ...
    public function storeManual(Request $request)
    {
        ...
        $items = AtkItem::query()->forModule(AtkItem::MODULE_ATK_MKS)->available()
        ...
        $atkMksRequest = AtkRequest::createPending($user, $rows, $validated['notes'] ?? null, AtkRequest::MODULE_ATK_MKS);

        return redirect()->route('v2.atk-mks.admin.requests.show', $atkMksRequest)
            ->with('success', 'Pengambilan manual berhasil dibuat. Silakan periksa setiap barang.');
    }

    public function show(AtkRequest $atkRequest)
    {
        $this->ensureAtkMksRequest($atkRequest);
        $atkRequest->load('items.item', 'user');

        return view('atk-mks.admin.requests.show', compact('atkRequest'));
    }
    ...
    public function reviewItem(Request $request, AtkRequest $atkRequest, AtkRequestItem $requestItem)
    {
        ...
        if ($validated['status'] === AtkRequestItem::STATUS_APPROVED) {
            $item = AtkItem::query()->forModule(AtkItem::MODULE_ATK_MKS)->available()->find($requestItem->atk_item_id);
    ...
    public function reject(Request $request, AtkRequest $atkRequest)
    {
        ...
        try {
            DB::transaction(function () use ($validated, $request, $atkRequest): void {
                $lockedRequest = AtkRequest::query()->forModule(AtkRequest::MODULE_ATK_MKS)
            ...
            return redirect()->route('v2.atk-mks.admin.requests.show', $atkRequest)->with('warning', $exception->getMessage());
        }

        return redirect()->route('v2.atk-mks.admin.requests.show', $atkRequest)->with('success', 'Pengajuan ditolak.');
    }
    ...
    public function approveAll(Request $request, AtkRequest $atkRequest)
    {
        ...
        try {
            DB::transaction(function () use ($request, $atkRequest): void {
                ...
                foreach ($required as $itemId => $qty) {
                    $item = AtkItem::query()->forModule(AtkItem::MODULE_ATK_MKS)->available()
        ...
        return redirect()->route('v2.atk-mks.admin.requests.show', $atkRequest)
    ...
    private function ensureAtkMksRequest(AtkRequest $atkRequest): void
    {
